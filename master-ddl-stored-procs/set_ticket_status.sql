...
DROP PROCEDURE commhub_junction.set_ticket_status;
CREATE PROCEDURE commhub_junction.set_ticket_status(
	IN in_end_user_token VARCHAR(32),
	IN in_workspace_token VARCHAR(32),
	IN in_ticket_id INT,
	IN in_new_status INT
    )
proc_label:BEGIN
...
-- all good so far 				
	UPDATE commhub_junction.work_ticket SET work_ticket_status_id = in_new_status WHERE work_ticket_id = in_ticket_id AND assigned_to_user_id = requesters_id;
