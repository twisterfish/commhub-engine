...
DROP PROCEDURE commhub_junction.set_ticket_running_time;
CREATE PROCEDURE commhub_junction.set_ticket_running_time(
...
-- all good so far 				
	UPDATE commhub_junction.work_ticket SET running_time = in_new_time, work_ticket_status_id = 2 WHERE work_ticket_id = in_ticket_id AND assigned_to_user_id = requesters_id AND work_ticket_status_id != 6;
	
-- if nothing changed then requester isn't the one working on the ticket and can't set a running time
	IF ROW_COUNT() = 0 THEN	 
		SET out_status = "invalid_not_authorized";
		SELECT out_status;		
		LEAVE proc_label;
