DELIMITER $$
DROP PROCEDURE commhub_junction.get_product_cost;

CREATE PROCEDURE commhub_junction.get_product_cost(
...
proc_label:BEGIN
    ...
    SELECT product_id, cost FROM commhub_junction.product_cost_history WHERE product_id = in_product_id AND effective_date <= FROM_UNIXTIME(in_effective_date) ORDER BY effective_date DESC LIMIT 1;
